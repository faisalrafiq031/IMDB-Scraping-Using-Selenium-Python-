...
def scrape_imdb_top_250():
    url = 'https://www.imdb.com/chart/top/?sort=popularity%2Casc'
...
def insert_into_database(movies):
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='IMDB_Top_250_Movies_By_popularity' AND xtype='U')
    BEGIN
        CREATE TABLE IMDB_Top_250_Movies_By_popularity (
    ...
    for movie in movies:
        cursor.execute('''
            INSERT INTO IMDB_Top_250_Movies_By_popularity (Title, Year, MovieTime, Runtime, IMDB_Rating, Rating_Count, Description, Director, Stars, ImageURL)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', 
        movie['Title'], 
        movie['Year'], 
...
if __name__ == '__main__':
    ...
    if movies:
        print(f"Scraped {len(movies)}  movies By Popularity successfully! ")
