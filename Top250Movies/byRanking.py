...
def insert_into_database(movies):
    ...
    for movie in movies:
        cursor.execute('''
            INSERT INTO IMDB_Top_250_Movies_By_Rank (Title, Year, MovieTime, Runtime, IMDB_Rating, Rating_Count, Description, Director, Stars, ImageURL)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', 
        movie['Title'], 
        movie['Year'], 
...
if __name__ == '__main__':
    ...
    if movies:
        print(f"Scraped {len(movies)}  movies By Ranking successfully! ")
