...
def insert_into_database(movies):
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='IMDB_Top_250_Movies_By_numRate' AND xtype='U')
    BEGIN
        CREATE TABLE IMDB_Top_250_Movies_By_numRate (
    ...
    for movie in movies:
        cursor.execute('''
            INSERT INTO IMDB_Top_250_Movies_By_numRate (Title, Year, MovieTime, Runtime, IMDB_Rating, Rating_Count, Description, Director, Stars, ImageURL)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', 
        movie['Title'], 
        movie['Year'], 
...
if __name__ == '__main__':
    ...
    if movies:
        print(f"Scraped {len(movies)}  movies By Number of Ratings successfully! ")
