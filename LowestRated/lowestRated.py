...
def insert_into_database(movies):
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='IMDB_Top_lowRate' AND xtype='U')
    BEGIN
        CREATE TABLE IMDB_Top_lowRate (
    ...
    for movie in movies:
        cursor.execute('''
            INSERT INTO IMDB_Top_lowRate (Title, Year, MovieTime, Runtime, IMDB_Rating, Rating_Count, Description, Director, Stars, ImageURL)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', 
        movie['Title'], 
        movie['Year'], 
...
if __name__ == '__main__':
    ...
    if movies:
        print(f"Scraped Top {len(movies)} Low Rated Movies successfully!")
