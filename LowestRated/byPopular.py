...
def scrape_imdb_top_250():
    url = 'https://www.imdb.com/chart/bottom/?sort=popularity%2Casc'
...
def insert_into_database(movies):
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='IMDB_Top_lowRate_bypopularity' AND xtype='U')
    BEGIN
        CREATE TABLE IMDB_Top_lowRate_bypopularity (
    ...
    for movie in movies:
        cursor.execute('''
            INSERT INTO IMDB_Top_lowRate_bypopularity (Title, Year, MovieTime, Runtime, IMDB_Rating, Rating_Count, Description, Director, Stars, ImageURL)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', 
        movie['Title'], 
        movie['Year'], 
...
if __name__ == '__main__':
    ...
    if movies:
        print(f"Scraped Top {len(movies)} Low Rated by Popularity Movies successfully!")
