...
def scrape_imdb_top_celebs():
    url = 'https://www.imdb.com/chart/starmeter/?ref_=chttentp_ql_8&sort=popularity%2Casc'
...
def insert_into_database(celebrities):
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='IMDB_Popular_Celebs_starMeter' AND xtype='U')
    BEGIN
        CREATE TABLE IMDB_Popular_Celebs_starMeter  (
    ...
    for celeb in celebrities:
        cursor.execute(''' 
            INSERT INTO IMDB_Popular_Celebs_starMeter (Name, Professions, Ranking_Info, Ranking_Category, Ranking_Additional_Info, MovieName, Description)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', 
        celeb['Name'], 
        celeb['Professions'], 
...
if __name__ == '__main__':
    ...
    if celebrities:
        print(f"Scraped {len(celebrities)} Popular Celebrities Data by StarMeter successfully!")
