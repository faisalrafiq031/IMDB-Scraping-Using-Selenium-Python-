...
def scrape_imdb_top_250():
    url = 'https://www.imdb.com/chart/toptv/?ref_=chttp_ql_6&sort=alpha%2Casc'
...
def insert_into_database(movies):
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='IMDB_Top_250_Shows_alphabetic' AND xtype='U')
    BEGIN
        CREATE TABLE IMDB_Top_250_Shows_alphabetic (
    ...
    for movie in movies:
        cursor.execute('''
            INSERT INTO IMDB_Top_250_Shows_alphabetic 
...
if __name__ == '__main__':
    ...
    if movies:
        print(f"Scraped Top {len(movies)} TvShows by Alphabetically Successfully!")
